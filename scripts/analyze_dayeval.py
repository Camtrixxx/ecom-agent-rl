...
def main() -> int:
    ...
    # 08-16 07:55 事后追加。预注册的阈值和分支一个字不改（下面照跑），但这个对照数要换：
    # #29 由四个 bootstrap 半宽反解出单次评测噪声 σ_m ≈ 0.70 pp，#14 那个 df=2 的锚偏小。
    # δ 是两次评测之差 → **权重相关分量为零时，s(δ) 的期望值就是 √2 σ_m = 0.99 pp**。
    print(f"  **但 0.39 pp 这个锚 08-16 已停用**（#29：σ_m ≈ 0.70 pp）。δ 是两次评测之差，")
    print(f"  所以零权重相关分量时 s(δ) 的期望值 = √2 × 0.70 = **0.99 pp**；实测 "
          f"{s_five * 100:.2f} pp 在它以下")
    print(f"  → 这个分量**和零分不开**，不要读成「已测到 0.6 pp 的权重相关漂移」。")
    print()
    if s_five < 0.007:
