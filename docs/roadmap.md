...
400，前面那节的分析被实测确认。（每回合的压缩次数没有单独记录，只有批级计数，所以这里
按批报；见阶段 D 末尾。）
...
它。三者都预测「绕圈回合更长、被压缩更多」。当前每回合的压缩次数没有单独记录
（`rollout/llm.py` 只有批级 `UsageTracker`），做不了等步数对照。要定论得单独跑一组
`--context-window 32768` 的诊断——窗口一改就和 baseline 不同口径，不能混进主评测。
